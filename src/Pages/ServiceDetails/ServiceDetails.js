...
const ServiceDetails = () => {
  ...
  const handleReview = (event) => {
    ...
    // hit insert api
    fetch("https://a11-flytographer-server-side.vercel.app/review", {
      method: "POST",
      headers: {
  ...
  return (
    <Container>
        ...
        <Col lg={8}>
          ...
          {user?.uid ? (
            ...
            <>
              ...
              <Modal show={show} onHide={handleClose} animation={false}>
                <Modal.Body>
                  Sorry!! You are Not Loggedin. If You want to add Review About
                  This Service You must need to Login
                  <Nav.Link
                    className="my-2 text-white btn btn-danger"
                    as={Link}
                    to="/login"
                  >
                    Login
      ...
      <Row className="mx-0 justify-content-center ">
        <ServiceReview serVicesId={services.serviceId}></ServiceReview>
      </Row>
    </Container>
  );
};
